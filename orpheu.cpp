...
cell AMX_NATIVE_CALL Orpheu::GetFunctionFromEntity(AMX* amx,cell* params)
{
	...
	long object =  edict ? (long) edict->pvPrivateData : NULL;
...
cell AMX_NATIVE_CALL Orpheu::GetFunctionFromMonster(AMX* amx,cell* params)
{
	...
	long object =  entvars ? (long) entvars->euser4 : NULL;
	...
	{	
		...
		if(memoryStructure)
		{
			...
			if(libraryInfo)
			{
				...
				{
					*get_amxaddr(amx,params[5]) = lastAddress;
				}
			...
			if((expectedParams == paramsCount) || ((expectedParams+1) == paramsCount))
			{
				...
				switch(memoryStructure->type)
				{
					case MemTypeOffset:
					{
						...
						if((expectedParams+1) == paramsCount)
						{
							*get_amxaddr(amx,params[2+typeHandler->getIfByRefParamsCount()]) = position;
						}
			...
			if(memoryStructure->type != MemTypeSearch)
			{
				...
				switch(memoryStructure->type)
				{
					case MemTypeOffset:
					{
						...
						if((expectedParams+1) == paramsCount)
						{
							*get_amxaddr(amx,params[4]) = position;
						}
